...
class T5_Tokenizer:
    def __init__(self, args):
        ...
        if args.run_mode == "train":
            self.subtokenizer.add_tokens(['<|NEG|>', '<|POS|>'])
    ...
    def convert_docpair_to_ids(self, pos_doc, neg_doc, max_length, query=None):
        ...
        if query is not None:
            label_ids = self.subtokenizer.encode(query) + [self.subtokenizer.eos_token_id]
            return input_ids, label_ids
        
        return input_ids
    
            
    def convert_doc_to_ids(self, pos_doc, max_length, query=None):
        pos_doc_ids = self.subtokenizer.encode(pos_doc, max_length=max_length)
...
def t5_train_pair_converter(
    ...
    """
    input_ids, label_ids = tokenizer.convert_docpair_to_ids(
        query=ex["query"],
        pos_doc= ex["pos_doc"], 
        neg_doc=ex["neg_doc"], 
...
def t5_train_single_converter(
    index, 
    ex, 
    tokenizer, 
    max_doc_len=(T5_MAX_LEN - 2)
):
    input_ids, label_ids = tokenizer.convert_doc_to_ids(
        query=ex["query"],
        pos_doc= ex["pos_doc"],
        max_length=max_doc_len,
    )
...
def t5_batchify_for_train(batch):
    
    indexs = [ex["index"] for ex in batch]
    inputs = [ex["input_ids"] for ex in batch]
    labels = [ex["label_ids"] for ex in batch]
    
    # pack batch tensor
    input_tensor = pack_batch_tensor(inputs)
    label_tensor = pack_batch_tensor(labels)

    return {
        "indexs":indexs,
        "input_ids":input_tensor["input_ids"], 
        "input_mask":input_tensor["input_mask"], 
        "label_ids":label_tensor["input_ids"], 
        "label_mask":label_tensor["input_mask"], 
    }
